...
xmid=$(( $xpitch / 2 ))
ymid=1000
$DIR/magicwand.sh $xmid,$ymid -t 25 -c transparent $outdir/$base-$id-dither.png $outdir/$base-$id-ditherwand.png
convert $outdir/$base-$id-ditherwand.png -trim $outdir/$base-$id-crop.png
...
brx=`convert $outdir/$base-$id-crop.png -format "%[fx:$ulx+$width-1]" info:`
bry=`convert $outdir/$base-$id-crop.png -format "%[fx:$uly+$height-1]" info:`
# echo "ulx=$ulx; uly=$uly; brx=$brx; bry=$bry;"
cr=$width"x"$height$ulx$uly
# echo $cr
#convert $in -crop $cr -fuzz 50% -transparent white $outdir/$base-$id-crop.png
# convert $in -crop $cr -fuzz 10% -transparent '#b8b8b8' -fuzz 10% -transparent '#707070' -fuzz 10% -transparent '#a0a0a0' -fuzz 10% -transparent '#d0d0d0' -fuzz 10% -transparent white -fuzz 10% $outdir/$base-$id-crop.png

convert  $outdir/$base-$id-crop.png -gravity center -background transparent -extent 1250x1250 $outdir/$base-$id-sq.png
convert $outdir/$base-$id-crop.png -scale 1x1! -alpha off $outdir/$base-$id-1x1.png;
echo -n "$base-$id "
